...
function fromRow(row) {
  return {
    ...
    garage: row.garage,
    price: row.price,
    basePersons: row.base_persons ?? 2,
...
// Every extra guest beyond a house's base_persons adds this flat amount per night.
export const EXTRA_PERSON_PRICE = 7;

export function calculateTotalPrice(basePrice, basePersons, persons) {
  const extraPersons = Math.max(0, Number(persons) - Number(basePersons));
  return Number(basePrice) + extraPersons * EXTRA_PERSON_PRICE;
}
